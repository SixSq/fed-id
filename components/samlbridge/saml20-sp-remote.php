<?php

$metadata[getenv('FP_BASEURL') . '/auth/realms/master'] = array (
    'AssertionConsumerService' => getenv('FP_BASEURL') . '/auth/realms/master/broker/eduGAIN/endpoint',
    'SingleLogoutService' =>  getenv('FP_BASEURL') . '/auth/realms/master/broker/eduGAIN/endpoint',
    'NameIDFormat' => 'urn:oasis:names:tc:SAML:2.0:nameid-format:persistent',
    'certData' => '',
    'signature.algorithm' => 'http://www.w3.org/2001/04/xmldsig-more#rsa-sha256',
    'validate.authnrequest' => false,
    'validate.logout' => false,
    'redirect.sign' => true,
    'redirect.validate' => true,
);
